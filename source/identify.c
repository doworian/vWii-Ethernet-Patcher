// IOS patching from postloader (thanks stfour)
...
// vWii specific
const u8 Kill_AntiSysTitleInstallv3_pt1_old[] = { 0x68, 0x1A, 0x2A, 0x01, 0xD0, 0x05 };
const u8 Kill_AntiSysTitleInstallv3_pt1_patch[] = { 0x68, 0x1A, 0x2A, 0x01, 0x46, 0xC0 };
const u8 Kill_AntiSysTitleInstallv3_pt2_old[] = { 0xD0, 0x02, 0x33, 0x06, 0x42, 0x9A, 0xD1, 0x01 };
const u8 Kill_AntiSysTitleInstallv3_pt2_patch[] = { 0x46, 0xC0, 0x33, 0x06, 0x42, 0x9A, 0xE0, 0x01 };
...
{
	write16((vu32)0xCD8B420A, 0); // mem protect off
	...
	write16((vu32)0xCD8B420A, 1); // mem protect on
}


// ticket check patch from postloader
#define ES_MODULE_START	(u16*)0x939F0000

// ES ticket check sequence, patched to grant full AHBPROT access
...
void PatchAHB()
{
	...
	for(patchme = ES_MODULE_START; patchme < ES_MODULE_START + 0x4000; patchme++)
	{
		...
		{
			// direct write needed, write16/uncached poke doesn't work here
			patchme[4] = 0x23FF;
